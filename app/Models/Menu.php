...
/* ADMIN MENU LIST SECTION */
        ...
        foreach($menu as $item) {
            ...
            if($item->parent_id == $parentId) {
                ...
                $result .= "<li class='OLO dd-item' data-id='{$item->id}'>
        ...
        foreach($menu as $item) {
            ...
            if($item->parent_id == $parentId) {
                ...
                $result .= "<li class='menu-item " . (($childItem) ? 'dropdown' : null) . (($childItem && $item->parent_id != 0) ? ' dropdown-submenu' : null) . "'>
                                <a href='" . url($item->url) . "' " . (($childItem) ? 'class="dropdown-toggle" data-toggle="dropdown"' : null) . ">{$item->title}" . (($childItem && $item->parent_id == 0) ? '<b class="caret"></b>' : null) . " COWBOY</a>" . $this->generateFrontMenu($menu, $item->id) . "
                            </li>";
            }
        }
